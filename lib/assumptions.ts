...
/**
 * שכר ממוצע במשק 2026 — קיימים שני ערכים שונים בחוק הביטוח הלאומי:
 *
 *   AVG_WAGE_SECTION1_2026 = ₪13,566 — סעיף 1. בסיס לחישוב דמי ביטוח
 *     לאומי / בריאות (3.1% מתחת ל-60% / 5% מעל). כלומר "כמה הפועל משלם".
 *
 *   AVG_WAGE_SECTION2_2026 = ₪13,769 — סעיף 2. בסיס לתקרות פנסיה
 *     ולתקרות גמלאות (קצבת זקנה, נכות, סיעוד, אבטלה — מה שהמדינה משלמת).
 *
 * הערך הישן `AVG_WAGE_2026` נשמר כ-alias לסעיף 2 לתאימות אחורית — רוב
 * השימושים בפרויקט (תקרות פנסיה ב-SECTION_45A_47_MONTHLY_CEILING, חישוב
 * PENSION_DISABILITY_SALARY_CAP) הם בקונטקסט פנסיה.
 *
 * לעדכן פעם בשנה (בדרך כלל סביב ינואר). מקור: btl.gov.il, שני הסעיפים
 * מתעדכנים בנפרד.
 */
export const AVG_WAGE_SECTION1_2026 = 13_566;
export const AVG_WAGE_SECTION2_2026 = 13_769;
/** @deprecated use AVG_WAGE_SECTION2_2026 explicitly. Kept as alias for back-compat. */
export const AVG_WAGE_2026 = AVG_WAGE_SECTION2_2026;
