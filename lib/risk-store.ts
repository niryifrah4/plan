...
export const RISK_CATEGORIES: RiskCategory[] = [
  {
    key: "death",
    label: "פטירה — ביטוח חיים",
    icon: "favorite",
    description: "כיסוי למקרה פטירה — הגנה על המשפחה",
  },
  {
    key: "disability",
    label: "נכות / אובדן כושר עבודה",
    icon: "accessible",
    description: "הבטחת הכנסה במקרה של אי-יכולת לעבוד",
  },
  {
    key: "nursing",
    label: "סיעוד",
    icon: "elderly",
    description: "כיסוי למקרה סיעוד — ביטוח פרטי וקופת חולים",
  },
  {
    key: "health",
    label: "בריאות",
    icon: "local_hospital",
    description: 'ביטוח בריאות פרטי, השלמת שב"ן, טכנולוגיות',
  },
  {
    key: "critical",
    label: "מחלות קשות",
    icon: "emergency",
    description: "כיסוי חד-פעמי למקרה אבחון מחלה קשה",
  },
  {
    key: "property",
    label: "רכוש ואחריות",
    icon: "home_work",
    description: "ביטוח דירה, רכב, אחריות מקצועית, שותפים",
  },
  ...
  // צוואה / הסכם ממון, לערוך". Drives questionnaire follow-up tasks.
  {
    ...
    label: "תכנון משפטי",
    icon: "gavel",
    description: "צוואה, הסכם ממון, ייפוי כוח מתמשך",
  },
];
...
let _id = 0;
const item = (category: string, label: string, description?: string): RiskItem => ({
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed)) {
        ...
        if (legalDefaults.length > 0) {
          const augmented = [
            ...parsed,
            ...legalDefaults.map((d) => ({ ...d, id: `risk_legal_${Date.now()}_${d.label}` })),
          ];
