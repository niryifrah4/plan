  ...
  useEffect(() => {
    ...
    if (
      ...
      (import.meta.env.VITE_DEV_AUTH_BYPASS === "1" ||
        localStorage.getItem("e2e:auth-bypass") === "1")
    ) {
      setBootstrapReady(true);
      return;
    }
    void (async () => {
      // Remote bootstrap must never hold the entire client UI hostage. Local
      // scoped data remains usable while Supabase is slow/offline; the remote
      // watcher will hydrate it in the background when connectivity returns.
      const ready = await Promise.race([
        prepareSessionScopeOnce("desktop-mount", lockedHouseholdId),
        new Promise<boolean>((resolve) => window.setTimeout(() => {
          console.warn("[bootstrap] timed out; rendering client shell with local data");
          resolve(true);
        }, 5000)),
      ]);
      if (
