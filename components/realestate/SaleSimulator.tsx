...
export function SaleSimulator({ property, allProperties, onClose }: Props) {
  ...
  const taxColor =
    result.taxStatus === "exempt"   ? "#1B4332" :
    result.taxStatus === "exempt_partial" ? "#B45309" :
    result.taxStatus === "overlap"  ? "#B45309" :
    result.taxStatus === "taxable"  ? "#8B2E2E" : "#5a7a6a";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 border-b v-divider flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            ...
            <div className="text-[11px] text-verdant-muted mt-0.5">
              ...
              {property.mortgageBalance ? ` · יתרת משכנתא: ${fmtILS(property.mortgageBalance)}` : ""}
            </div>
          </div>
        ...
        {/* Results */}
        <div className="px-6 py-5 border-t v-divider space-y-3" style={{ background: "#F9FAF2" }}>
          {/* The big number */}
          <div className="text-center py-4 rounded-2xl"
            ...
            <div className="text-[42px] font-extrabold tabular-nums leading-none mt-2"
            ...
            <div className="text-[12px] mt-2" style={{ color: "rgba(255,255,255,0.85)" }}>
              ב-{new Date(result.saleDate).toLocaleDateString("he-IL", { month: "long", year: "numeric" })}
          ...
          {/* Context */}
          <div className="text-[11px] text-verdant-muted leading-relaxed px-1">
            הערכה אינדיקטיבית בלבד. מס שבח חושב על הרווח הנומינלי (ללא הצמדה למדד —
            ההפחתה האמיתית עשויה להיות נמוכה יותר). עמלות תיווך משתנות לפי הסוכן.
          </div>
        </div>
...
function Row({
  label, value, negative, bold, color,
}: {
  label: string; value: number; negative?: boolean; bold?: boolean; color?: string;
}) {
