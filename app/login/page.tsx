...
export default function LoginPage() {
  ...
  // 2026-05-04: signup is now public (B2C course buyers register themselves).
  // ?admin=1 in the URL flags an admin/advisor signup — passes
  // signup_role='advisor' to the auth trigger so it creates an advisor row
  // instead of the default household.
  ...
  useEffect(() => {
    ...
    // Surface error params from /auth/callback (e.g. ?error=missing_role).
        ...
        const { error } = await sb.auth.signUp({
          ...
          options: {
            data: {
              ...
              // ?admin=1 → advisor signup. Default → household (couple).
  ...
  const handleGoogleLogin = async () => {
    ...
    await sb.auth.signInWithOAuth({
      ...
      options: {
        ...
        // Persist signup metadata so the auth trigger picks the right path.
        // For existing users (login), the trigger doesn't fire — these are
        // ignored. For new signups, they steer the flow:
        //   invite_token → linked to advisor's household
        //   signup_role=advisor → admin/advisor creation (?admin=1)
        //   neither → default (course buyer / household self-signup)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: "24px" }} className="text-center">
          ...
          >
            {mode === "login"
              ...
              : isAdminSignup
                ? "צור חשבון יועץ והתחל לנהל את הלקוחות שלך"
                : "צרו חשבון והתחילו לתכנן את העתיד הכספי שלכם"}
          </p>
        </div>
        ...
        {/* Mode switcher.
            2026-05-04: signup is now public (B2C course buyers register on
            their own). The ?admin=1 URL flag turns the form into advisor
            signup, used for adding new advisors to the system. */}
        <div
        ...
        {!isDemoMode && (
          <>
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 transition-all hover:shadow-md"
              style={{
                marginTop: "16px",
                height: "48px",
                borderRadius: "9999px",
        ...
        <div className="mt-6 text-center text-[11px] text-verdant-muted">
          <a href="/terms" className="hover:text-verdant-emerald hover:underline">
            תנאי שימוש
          </a>
          <span className="mx-2">·</span>
          <a href="/privacy" className="hover:text-verdant-emerald hover:underline">
            מדיניות פרטיות
          </a>
        </div>
      </div>
