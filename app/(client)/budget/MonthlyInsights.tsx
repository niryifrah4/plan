  ...
  0: [
    {
      ...
      adjustments: [
        { sectionKey: "fixed", rowName: "ביטוחים", field: "budget", multiplier: 1.05, label: "ביטוחים +5% (עדכון שנתי)" },
      ],
    },
  ],
  1: [
  ...
  2: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "סופר / מזון", field: "budget", multiplier: 1.25, label: "מזון +25% (קניות פסח)" },
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.20, label: "פנאי +20% (חופשת חג)" },
        { sectionKey: "variable", rowName: "מסעדות", field: "budget", multiplier: 1.15, label: "מסעדות +15% (אירוח חג)" },
      ],
    },
  ],
  3: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "סופר / מזון", field: "budget", multiplier: 1.30, label: "מזון +30% (פסח)" },
        { sectionKey: "variable", rowName: "ביגוד / קניות", field: "budget", multiplier: 1.25, label: "ביגוד +25% (קניות חג)" },
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.35, label: "פנאי +35% (חול המועד)" },
        { sectionKey: "variable", rowName: "דלק / תחבורה", field: "budget", multiplier: 1.15, label: "דלק +15% (טיולי חג)" },
      ],
    },
  ],
  4: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.15, label: "פנאי +15% (חגים)" },
        { sectionKey: "variable", rowName: "סופר / מזון", field: "budget", multiplier: 1.10, label: "מזון +10% (אירוח)" },
      ],
    },
  ],
  5: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.20, label: "פנאי +20% (תכנון קיץ)" },
      ],
    },
  ],
  6: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.40, label: "פנאי +40% (חופשות קיץ)" },
        { sectionKey: "variable", rowName: "מסעדות", field: "budget", multiplier: 1.20, label: "מסעדות +20% (ימי חופש)" },
        { sectionKey: "variable", rowName: "דלק / תחבורה", field: "budget", multiplier: 1.20, label: "דלק +20% (טיולים)" },
      ],
    },
  ],
  7: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "ביגוד / קניות", field: "budget", multiplier: 1.30, label: "ביגוד +30% (ציוד לימודים)" },
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.25, label: "פנאי +25% (קייטנות)" },
      ],
    },
  ],
  8: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "סופר / מזון", field: "budget", multiplier: 1.25, label: "מזון +25% (ר״ה)" },
        { sectionKey: "variable", rowName: "ביגוד / קניות", field: "budget", multiplier: 1.20, label: "ביגוד +20% (חגים + לימודים)" },
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.15, label: "פנאי +15% (חגי תשרי)" },
      ],
    },
  ],
  9: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.30, label: "פנאי +30% (סוכות)" },
        { sectionKey: "variable", rowName: "סופר / מזון", field: "budget", multiplier: 1.15, label: "מזון +15% (אירוח סוכות)" },
        { sectionKey: "variable", rowName: "דלק / תחבורה", field: "budget", multiplier: 1.15, label: "דלק +15% (חול המועד)" },
      ],
    },
  ],
  10: [
  ...
  11: [
    {
      ...
      adjustments: [
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.15, label: "פנאי +15% (חנוכה)" },
        { sectionKey: "fixed", rowName: "חשמל", field: "budget", multiplier: 1.20, label: "חשמל +20% (חימום חורף)" },
      ],
    },
  ],
};
...
function getChildrenInsight(month: number, childCount: number): SeasonalInsight | null {
  ...
  if (month === 6 || month === 7) {
    return {
      ...
      adjustments: [
        { sectionKey: "fixed", rowName: "גן / חינוך", field: "budget", multiplier: 1 + (0.15 * childCount), label: `חינוך +${(15 * childCount)}% (${childCount} ילדים בקיץ)` },
        { sectionKey: "variable", rowName: "פנאי ובילוי", field: "budget", multiplier: 1.25, label: "פנאי +25% (פעילויות ילדים)" },
      ],
    };
  }
  ...
  if (month === 8) {
    return {
      ...
      adjustments: [
        { sectionKey: "fixed", rowName: "גן / חינוך", field: "budget", multiplier: 1 + (0.10 * childCount), label: `חינוך +${(10 * childCount)}% (פתיחת לימודים)` },
        { sectionKey: "variable", rowName: "ביגוד / קניות", field: "budget", multiplier: 1.20, label: "ביגוד +20% (ציוד לימודים)" },
      ],
    };
  }
...
function isSelfEmployed(fields: Record<string, string>): boolean {
  ...
  const selfTerms = ["עצמאי", "עצמאית", "עוסק מורשה", "עוסק פטור", "חברה בע\"מ", "פרילנס", "freelance", "self"];
...
export default function MonthlyInsights({ month, year, onApply }: Props) {
  ...
  const allAdjustments = useMemo(
    () => insights.flatMap(i => i.adjustments),
    [insights],
  );
  ...
  return (
    <section
      className="rounded-2xl p-5 md:p-7 mb-4 transition-all"
      style={{
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-[10px] font-bold uppercase tracking-[0.15em] mt-0.5" style={{ color: "#5a7a6a" }}>
        ...
        <button
          ...
          className="text-[11px] font-bold px-2 py-1 rounded-lg transition-colors hover:bg-white/60"
          style={{ color: "#5a7a6a" }}
          title="הסתר תובנות"
        >
      ...
      {allAdjustments.length > 0 && (
        <div className="flex items-center justify-between mt-4 pt-3" style={{ borderTop: "1px solid #d1fae5" }}>
          <div className="text-[11px] font-semibold" style={{ color: "#5a7a6a" }}>
            {allAdjustments.length} התאמות מוצעות לשדות התכנון
          </div>
          {applied ? (
            <div
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[12px] font-extrabold"
