...
function feeBenchmark(fee: number): { color: string; label: string } {
  ...
  if (fee <= 0.5) return { color: "#B45309", label: "סביר" };
  return { color: "#8B2E2E", label: "גבוה" };
}
...
export default function PensionPage() {
  ...
  const pensionAssetClassBreakdown = useMemo(() => {
    ...
    const colors: Record<string, string> = { equity: "#1B4332", bonds: "#1a6b42", cash: "#2B694D", alternative: "#B45309" };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
        ...
        <SolidKpi label="דמי ניהול ממוצעים" value={`${weightedFee.toFixed(2)}%`} icon="percent"
                  tone={feeBenchmark(weightedFee).color === "#8B2E2E" ? "red" : feeBenchmark(weightedFee).color === "#1B4332" ? "emerald" : "amber"}
      ...
      <section className="v-card overflow-hidden mb-6">
        ...
        {insuranceDuplication && (
          <div className="mx-5 mt-3 p-3 rounded-lg flex items-center gap-2 text-xs" style={{ background: "#fef3c7", border: "1px solid #B45309" }}>
            <span className="material-symbols-outlined text-[16px]" style={{ color: "#B45309" }}>warning</span>
        ...
        {funds.some(f => f.subtype === "bituach_classic" || f.subtype === "bituach_adif" || f.subtype === "pension_vatika") && (
          <div className="mx-5 mt-2 p-3 rounded-lg flex items-start gap-2 text-xs"
            ...
            <span className="material-symbols-outlined text-[16px] mt-0.5" style={{ color: "#B45309" }}>lock</span>
            <div>
              <span className="font-bold" style={{ color: "#92400e" }}>
      ...
      {studyFundWarning && (
        <div className="card-pad mb-6" style={{ borderInlineStart: "4px solid #8B2E2E" }}>
          <div className="flex items-start gap-3">
            <span className="material-symbols-outlined text-[22px] flex-shrink-0" style={{ color: "#8B2E2E" }}>warning</span>
            <div className="flex-1 min-w-0">
