...
import { addManualTransaction, loadParsedTransactions } from "@/lib/budget-import";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {categories.length === 0 ? (
          <div style={emptyChipsBox}>אין קטגוריות מוגדרות עדיין.</div>
        ) : (
          <div
            style={{
              marginTop: 6,
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: active ? "var(--morning-leaf-tint)" : "var(--morning-surface)",
...
const emptyChipsBox: React.CSSProperties = {
  marginTop: 6,
  marginBottom: 14,
  fontSize: 13,
  color: "var(--morning-muted)",
...
/* ─────────────────────────────────────────────── */
/* Numpad amount entry — replaces the keyboard    */
/* input for the AddExpenseSheet's amount step.   */
/* Built per ui-agent rec: drop-in 5-second flow. */
/* ─────────────────────────────────────────────── */
...
function NumpadAmount({
  ...
  const append = (ch: string) => {
    ...
    if (value.includes(".") && value.split(".")[1]?.length >= 2 && ch !== ".") {
      return;
    }
    // Leading zero handling: replace "0" with the digit unless it's "."
    if (value === "0" && ch !== ".") {
      onChange(ch);
      return;
    }
