...
for (const p of bundle.policies) {
  ...
  console.log('   annualDeposits:', p.annualDeposits?.toLocaleString(), '| monthly:', p.monthlyContrib);
  console.log('   fees: bal%=', p.mgmtFeeBalance, ' dep%=', p.mgmtFeeDeposit);
  console.log('   returns: 1Y=', p.returnYear, '%  5Y=', p.return5y, '%');
  console.log('   pension projected:', p.projectedPensionAmount?.toLocaleString(), '@ age', p.retirementAge);
